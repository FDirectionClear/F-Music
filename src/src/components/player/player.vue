		...
		<transition name = "normal"
			...
			<div class="bottom">
				<div class="dot-wrapper">
					<span v-for = "index in 2" class="dot" :class = "{ 'active' : dotIndex === index }"></span>
				</div>
		...
		<transition name = "normal">
			...
			<div class="icon">
				<div class="imgWrapper">
					<img width="40" height="40" :src = "currentSong.image" ref = "miniCD" :class = "cdRotate">
				</div>
        	</div>
			<div class="text">
...
import { prefixStyle, getTranslateVal } from 'common/js/domOps'
...
export default {
	...
	data () {
		return {
			currentTime: 0, // 当前播放器正在变化的时间，与audio的currentTime绑定
			readyTime: 0, // 预备变化的时间，没有预备时间则为0
			percentage: 0, //  当前播放时间的百分比，主要用于控制进度条的自动推进
			currentLyric:  null, // 当前歌曲的歌词（熟肉）
			currentLineNum: 0, //  当前歌词所在的行，默认是0
			touch: {}, // 用来储存手指信息
			dotIndex: 1 // 当前高亮小点点的索引
		}
	},
	computed: {
		playIcon () {
	...
	methods: {
		...
		format (time = this.currentTime) {
			if(time !== this.currentSong.duration) {
				// readyTime在显示上优先级更高, 同时防止time受到audio.currentTime错误影响成为null 或者 undefined 或 false
				// 如果真的发生错误，那么歌曲会播放，显示时间就一直为0:00
				time = this.readyTime ||  time || 0
			}
			// 如果当前时间和总时间一样长，那么就说明他是总时间，直接整理后返回即可
			const min = (time / 60) | 0
			const sec = (time % 60) | 0
		...
		dragBar (per) {
			...
			this._updateReadyTime(0)
			this.$refs.audio.currentTime = this.currentTime
			this._seekLyric(this.currentTime) // 当我们放开进度条，那么就要再次播放歌词。
			if(!this.playing) {
				// 松手后进入播放状态
				this.togglePlay()
			}
		...
		draggingBar (per) {
			this._updateReadyTime(per)
			this._seekLyric(this.readyTime)
		...
		touchStart (e) {
			const middleL = this.$refs.middleL,
				 	  middleR = this.$refs.scroll.$el
			const transitionVal = 'all .3s'
			if (!middleL.style.transition) {
				middleL.style.transition = transitionVal
			}
			if (!middleR.style.transition) {
				middleR.style.transition = transitionVal
			}
		...
		touchMove (e) {
			this.deltaX = e.touches[0].pageX - this.touch.startX  // 向右边的位移x
			this.deltaY = e.touches[0].pageY - this.touch.startY // 同时y方向的位移

			console.log(Math.abs(this.deltaX), Math.abs(this.deltaY))
			...
			const { X } = getTranslateVal(this.$refs.scroll.$el)
			console.log(this.deltaX, X)
			if(this.deltaX < 0 && ( X  && X === -window.innerWidth)) {
				// 如果当前已经完全移动到视口中，向右滑无效
				return
			}
			if(this.deltaX > 0 && (X !== undefined && X === 0)) {
				// 如果当前已经完全移动到视口中，向右滑无效
				return
			}
			this._middleRTranslate(this.deltaX)
		},
		touchEnd (e) {
			...
			this.deltaX < 0 // 从右往左
				? this.deltaX + MIN_DISTANCE > 0
					? this._middleRTranslate(0)
					: this._middleRTranslate(-innerWidth)
				:  this.deltaX - MIN_DISTANCE <= 0 // 从左往右
					? this._middleRTranslate(-innerWidth) // 归位
					: this._middleRTranslate(0)
		},
		_middleRTranslate (deltaX) {
			const innerWidth = window.innerWidth
			console.log(`应该位移${deltaX}，屏幕宽度${-innerWidth}`)
			if(deltaX <=0) {
				// 如果是从右往左，那么deltaX的大小就是要移动的大小
				if(deltaX  === -innerWidth) {
					this.dotIndex = 2
				}
				if(deltaX === 0) {
					this.dotIndex = 1
				}
				this.$refs.scroll.$el.style[TRANSFORM] = `translate3d(${deltaX}px, 0, 0)`
				this.$refs.middleL.style.opacity = `${1 + deltaX / innerWidth}`
			} else {
				// 如果是从左往右，那么deltaX的大小就需要window最差，否则会出现位移距离和手指移动距离不同的情况。
				this.$refs.scroll.$el.style[TRANSFORM] = `translate3d(${ deltaX - window.innerWidth}px, 0, 0)`
				console.log('执行')
				this.$refs.middleL.style.opacity = `${deltaX / innerWidth}`
			}
		},
		_getLyric () {
			this.currentSong.getLyric().then((lyric) => {
				...
				if (this.playing) {
					// Lyric初始化好后，如果当前状态为播放就让歌词和播放器同步播放。
		...
		_seekLyric (time) {
			this.currentLyric.seek(time * 1000) // 因为readyTime改变必然会改变歌词的位置
		},
		_updateReadyTime (per) {
			this.readyTime = per * this.currentSong.duration
		},
		_updateCurrTime (per) {
			...
			this.$nextTick(()=>{
				...
				newState
					? !function () {
							audio.play()
							if(currentLyric) {
								// playing的user watcher的回调初次触发时，currentLyric可能还不存在。
...
所遇到的知识坑点：
	...
	比如：
		...
		自动移除xxx-enter、xxx-enter-active，xxx-enter-to.....，因为移除这些类名是AfterBefore钩子执行完毕后Vue帮忙做的。
